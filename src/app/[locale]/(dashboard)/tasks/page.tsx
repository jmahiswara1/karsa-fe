...
export default function TasksPage() {
  ...
  return (
    <div className="flex h-full flex-col space-y-6 pb-2">
      ...
      {/* Greeting Banner */}
      <PageBanner user={user} subtitle={tPages('tasks_desc')} />
      ...
      {view === 'board' ? (
          ...
          {isLoading ? (
            <div className="flex h-full gap-6">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-muted/40 flex h-full w-80 shrink-0 flex-col gap-3 rounded-xl p-3.5"
                >
        ...
        <div className="min-h-[500px] flex-1 overflow-y-auto pb-4">
          <TaskList tasks={tasks} isLoading={isLoading} onEditTask={handleEditTask} sort={sort} />
        </div>
      )}
