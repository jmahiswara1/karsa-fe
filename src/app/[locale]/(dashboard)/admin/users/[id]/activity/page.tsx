...
export default function UserActivityPage() {
  ...
  const fetchActivities = useCallback(async () => {
    ...
    try {
      const res = await api.get(`/api/admin/users/${userId}/activities?${params.toString()}`);
  ...
  }, [userId, page, actionFilter, entityFilter]);
              ...
              {activities.map((activity) => {
                ...
                return (
                  <tr key={activity.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3">
                      <p className="max-w-[300px] truncate text-sm text-gray-600">{title || '—'}</p>
                    </td>
